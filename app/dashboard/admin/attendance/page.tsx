...
async function AttendanceData({ searchParams }: { searchParams: Promise<{ employee?: string }> }) {
  ...
  try {
    dbUser = await prisma.user.findUnique({ where: { id: user.id }, include: { employee: true } });
    ...
    if (companyId) {
      ...
      if (dbUser && dbUser.employee) {
        ...
        const todayRecord = await prisma.attendance.findFirst({
          where: { employeeId: dbUser.employee.id, date: { gte: today, lte: endOfDay } }
        });
        isCheckedIn = !!todayRecord?.checkInTime && !todayRecord?.checkOutTime;
      }
    }
  } catch (error) {
    console.warn("Prisma Database connection failed in Attendance.");
  }

  return (
    <div className="space-y-6">
      <AiInsightsPanel companyId={dbUser?.companyId ?? ''} userId={user.id} />
      <AttendanceClient initialLogs={initialLogs} isInitiallyCheckedIn={isCheckedIn} />
    </div>
  );
}
